...
async def get_similar_by_spot_id_in_global(spot_id: str,
                                           es: Any = Depends(get_es_client), collection: Any = Depends(get_m_db)):
    """
...
@router.get(path="/{spot_id}/local",
            response_model=List[SimpleSpotDto])
async def get_similar_by_spot_id_in_local(spot_id: str,
                                          es: Any = Depends(get_es_client), collection: Any = Depends(get_m_db)):
    """
    관광지의 Id를 받아 유사한 관광지를 10개 반환합니다.
